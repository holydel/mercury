...
void LoadVK_Library()
{
	gLibHandle = ll::os::gOS->LoadSharedLibrary(gVulkanRuntimeLibraryName);
	...
	auto libFullName = ll::os::gOS->GetSharedLibraryFullFilename(gLibHandle);
	MLOG_INFO(u8"Found vulkan loader: %s", libFullName.c_str());
}
...
{
	ll::os::gOS->UnloadSharedLibrary(gLibHandle);
	gLibHandle = nullptr;
}
